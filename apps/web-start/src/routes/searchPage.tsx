...
function RouteComponent() {
  ...
  const validateAddress = (address: string | null | undefined, city: string | null | undefined, state: string | null | undefined, zip: string | null | undefined): string | null => {
  ...
  const validateEditData = (): boolean => {
    ...
    // Validate customer address if any address field is provided
    if (editData.customerAddress || editData.customerCity || editData.customerState || editData.customerZip) {
      const customerError = validateAddress(editData.customerAddress, editData.customerCity, editData.customerState, editData.customerZip)
      if (customerError) {
        errors.customerAddress = customerError
      }
    }

    // Validate respondent address if any address field is provided
    if (editData.respondentAddress || editData.respondentCity || editData.respondentState || editData.respondentZip) {
      const respondentError = validateAddress(editData.respondentAddress, editData.respondentCity, editData.respondentState, editData.respondentZip)
      if (respondentError) {
          ...
          >
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                {!isEditing ? (
                  ...
                  <>
                    ...
                    <button
                      onClick={() => { setIsEditing(false); setEditData({}); setEditErrors({}) }}
                ...
                <button
                  onClick={() => { setIsModalOpen(false); setSelectedComplaint(null); setIsEditing(false); setEditData({}); setEditErrors({}) }}
            ...
            {!detailLoading && selectedComplaint && (
              <div style={{ marginTop: 15 }}>
                  ...
                  {isEditing ? (
                    <div style={{ marginTop: 5 }}>
                      <input
                        ...
                        style={{ width: '100%', padding: '6px', marginBottom: 5, borderRadius: 4, border: '1px solid #ccc' }}
                      />
                      <div style={{ display: 'flex', gap: 5 }}>
                        <input
                          ...
                          onChange={(e) => setEditData({ ...editData, customerCity: e.target.value })}
                        ...
                        <input
                          ...
                          onChange={(e) => setEditData({ ...editData, customerState: e.target.value })}
                          style={{ width: '60px', padding: '6px', borderRadius: 4, border: '1px solid #ccc' }}
                        />
                        <input
                          type="text"
                          placeholder="ZIP"
                          value={editData.customerZip || ''}
                          onChange={(e) => setEditData({ ...editData, customerZip: e.target.value })}
                          style={{ width: '80px', padding: '6px', borderRadius: 4, border: '1px solid #ccc' }}
                        />
                      </div>
                      {editErrors.customerAddress && (
                        <div style={{ color: 'red', fontSize: '12px', marginTop: 2 }}>{editErrors.customerAddress}</div>
                      )}
                    </div>
                  ) : (
                    <span> {selectedComplaint.customerAddress ? `${selectedComplaint.customerAddress}, ${selectedComplaint.customerCity}, ${selectedComplaint.customerState} ${selectedComplaint.customerZip}` : 'N/A'}</span>
                  )}
                </div>
                <div style={{ marginBottom: 10 }}>
                ...
                {selectedComplaint.respondentName && (
                  <div style={{ marginBottom: 10 }}>
                    ...
                    {isEditing ? (
                      <div style={{ marginTop: 5 }}>
                        <input
                          type="text"
                          placeholder="Street Address"
                          value={editData.respondentAddress || ''}
                          onChange={(e) => setEditData({ ...editData, respondentAddress: e.target.value })}
                          style={{ width: '100%', padding: '6px', marginBottom: 5, borderRadius: 4, border: '1px solid #ccc' }}
                        />
                        <div style={{ display: 'flex', gap: 5 }}>
                          <input
                            type="text"
                            placeholder="City"
                            value={editData.respondentCity || ''}
                            onChange={(e) => setEditData({ ...editData, respondentCity: e.target.value })}
                            style={{ flex: 1, padding: '6px', borderRadius: 4, border: '1px solid #ccc' }}
                          />
                          <input
                            type="text"
                            placeholder="State"
                            value={editData.respondentState || ''}
                            onChange={(e) => setEditData({ ...editData, respondentState: e.target.value })}
                            style={{ width: '60px', padding: '6px', borderRadius: 4, border: '1px solid #ccc' }}
                          />
                          <input
                            type="text"
                            placeholder="ZIP"
                            value={editData.respondentZip || ''}
                            onChange={(e) => setEditData({ ...editData, respondentZip: e.target.value })}
                            style={{ width: '80px', padding: '6px', borderRadius: 4, border: '1px solid #ccc' }}
                          />
                        </div>
                        {editErrors.respondentAddress && (
                          <div style={{ color: 'red', fontSize: '12px', marginTop: 2 }}>{editErrors.respondentAddress}</div>
                        )}
                      </div>
                    ) : (
                      <span> {selectedComplaint.respondentAddress ? `${selectedComplaint.respondentAddress}, ${selectedComplaint.respondentCity}, ${selectedComplaint.respondentState} ${selectedComplaint.respondentZip}` : 'N/A'}</span>
                    )}
                  </div>
                )}
                <div style={{ marginBottom: 10 }}>
